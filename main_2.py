...
SERIAL_PORT_1 = 'COM3'
SERIAL_PORT_1 = 'COM6'  # Замените на нужный порт
BAUD_RATE = 115200
RELAY_COMMAND = "OPEN_RELAY"

def send_command_to_controller(command, serial_port):
    try:
        with serial.Serial(serial_port, BAUD_RATE, timeout=1) as ser:
            ser.write((command + '\n').encode())
            print(f"Команда {command} отправлена на порт {serial_port}.")
    except serial.SerialException as e:
        print(f"Ошибка последовательного порта {serial_port}: {e}")
        ...
        if history_data:
            ...
            if entry_date_time is not None and exit_date_time is None:
                ...
                if time_diff >= MIN_INTERVAL:
                    ...
                    send_command_to_controller(RELAY_COMMAND, SERIAL_PORT_2)
                ...
                if time_since_exit >= MIN_INTERVAL:
                    ...
                    save_to_json_single(data)
                    send_command_to_controller(RELAY_COMMAND, SERIAL_PORT_1)
        else:
...
SERIAL_PORT_1 = 'COM3'  # Первый контроллер для первой камеры
SERIAL_PORT_2 = 'COM6'  # Второй контроллер для второй камеры
BAUD_RATE = 115200
RELAY_COMMAND = "OPEN_RELAY"

# Функция отправки команды на указанный контроллер
def send_command_to_controller(command, serial_port):
    try:
        with serial.Serial(serial_port, BAUD_RATE, timeout=1) as ser:
            ser.write((command + '\n').encode())
...
# Измененная функция process_stream для отправки команды на нужный контроллер
def process_stream(stream_id, video_file_path, detector, LPRnet, conn, seen_plates, output_size):
    global last_saved_time
    serial_port = SERIAL_PORT_1 if stream_id == 1 else SERIAL_PORT_2  # Выбор порта в зависимости от ID камеры
                ...
                if re.match(r"[A-Z]{1}[0-9]{3}[A-Z]{2}[0-9]{2,3}", plate_text):
                    ...
                    if plate_in_db and plate_text not in seen_plates:
                        if plate_text not in last_saved_time or (current_time - last_saved_time[plate_text]) >= timedelta(minutes=2):
                            data = {"LicensePlate": plate_text}
                            save_to_json_single(data)
                            last_saved_time[plate_text] = current_time
                            seen_plates.add(plate_text)

                            # Отправляем команду на контроллер, связанный с конкретным потоком (камерой)
                            send_command_to_controller(RELAY_COMMAND, serial_port)

        # Отрисовка рамок вокруг номеров
        drawn_frame = draw_number_boxes(cars, raw_frame) if cars else raw_frame
        yield drawn_frame

# Основная функция main остается без изменений
